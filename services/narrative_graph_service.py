...
def import_assets_to_graph(project_ref: str) -> NarrativeGraphResult:

    """从大纲（novel_outline.md）提取结构化内容导入图谱。



    解析规则（大纲由 prompt 模板强制为固定 Markdown 结构）：

    - `## 主要人物` 小节下的 `- **姓名（年龄）**：描述` → character 节点

    - `## 世界观设定` 小节下的 `**标题**：描述` → world_fact 节点



    跳过已存在的同名节点；不覆盖手动编辑的图谱数据。

    返回导入结果（含新增节点数与 message）。
    ...
    # 从项目目录直接读取大纲（与图谱文件同源，兼容测试注入的 ctx）
    ...
    if project_dir is not None:
        ...
        outline_paths = sorted(

            Path(project_dir).glob("novel_outline*.md"),
        ...
        if outline_paths:
            ...
            try:
                ...
                outline_text = outline_paths[-1].read_text(encoding="utf-8")
    ...
    if outline_text is None:
        ...
        return NarrativeGraphResult(
            ...
            message="Novel outline not found. Generate outline first.",
    ...
    existing_labels = {clean_text(node.get("label")) for node in graph["graph"]["nodes"]}
    ...
    def add_if_new(label: str, node_type: str, summary: str) -> None:

        clean_label = clean_text(label)

        if not clean_label or clean_label in existing_labels:
        ...
        node, error = build_graph_node_for_create(
            ...
            _generate_id(f"node_{node_type}", existing_ids),

            source={"created_by": "outline_import", "introduced_in": None, "last_updated_in": None},
        ...
        existing_labels.add(clean_label)

        created_nodes.append(node)



    # 1. 解析"主要人物"小节 → character 节点

    _import_character_nodes(outline_text, add_if_new)



    # 2. 解析"世界观设定"小节 → world_fact 节点

    _import_world_fact_nodes(outline_text, add_if_new)



    if not created_nodes:

        # 幂等：全部已存在时返回成功，仅提示无新增
        ...
        return NarrativeGraphResult(
            ...
            message=f"No new nodes imported from outline (skipped {len(skipped)}).",
    ...
    result = NarrativeGraphResult(
        ...
        message=f"Imported {len(created_nodes)} nodes from outline (skipped {len(skipped)}).",
    ...
    append_event_best_effort(
        ...
        event_type="graph_import",

        summary=f"Imported {len(created_nodes)} nodes from outline (skipped {len(skipped)}).",

        source={"imported": len(created_nodes), "skipped": len(skipped)},
...
def _import_character_nodes(outline_text: str, add_if_new) -> None:

    """从 `## 主要人物` 小节提取 `- **姓名（年龄）**：描述`。"""

    section = _outline_section(outline_text, "主要人物")
    ...
    for line in section.splitlines():
        ...
        # 去掉"（年龄）"后缀作为 label，年龄信息并入 summary

        label = re.sub(r"（.*?）$", "", name).strip() or name

        full_summary = f"{name}。{summary}".strip("。") if summary else name

        add_if_new(label, "character", full_summary)





def _import_world_fact_nodes(outline_text: str, add_if_new) -> None:

    """从 `## 世界观设定` 小节提取 `**标题**：描述`。"""

    section = _outline_section(outline_text, "世界观设定")
    ...
    for line in section.splitlines():
        ...
        add_if_new(title, "world_fact", summary or title)





def _outline_section(outline_text: str, section_title: str) -> str:

    """按 `## 标题` 切分大纲，返回指定小节（到下一个 ## 之前）。"""
