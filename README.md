...
一个本地运行的轻量 AI 小说生成器。当前正式前端是 Braipen：React 19 + FastAPI，通过 OpenAI 兼容的 DeepSeek API 完成长篇创作工作流。项目面向个人写作和长期迭代：结构清晰、没有数据库、没有用户系统。Streamlit 前端已废弃并删除，不再维护，也不再作为回归测试目标。
...
- 支持叙事图谱可视化：@antv/g6 画布拖拽布局、节点增删改、关系连线创建、从大纲一键导入（`POST /narrative-graph/import-assets`）。

- 支持 AI 推理过程展示：章节生成 / 对话式续写时以 `reasoning` 事件流式透传模型思考过程，仅展示、绝不写入文件。

- 支持对话式续写闭环：续写结果可一键保存回章节文件（`POST .../continue/save`，append / replace 两种模式）。

- 支持项目删除：`DELETE /api/projects/{project_ref}`（仅 workspace 项目）。
...
│   ├── common.py          # 公共工具（clean_text / 原子写 / 时间戳 / 项目上下文解析）

│   ├── generation_service.py

│   ├── narrative_graph_service.py

│   ├── project_service.py

│   └── ...

├── frontend/              # React 19 + Vite + AntD 6

│   ├── src/

│   │   ├── pages/         # 仪表盘 / 创作驾驶舱 / 阅读器 / 图谱 / 审查 / 资料库 / 设置

│   │   ├── components/

│   │   └── appConfig.ts   # 开屏动画等前端配置开关
...
Streamlit 前端已废弃并删除（`app.py` 已从仓库移除）。`start.bat` 只打印退役提示并转向 `start-react.bat`。所有新功能、UI 变更和回归测试只面向 React + FastAPI。
...
- 已由 React + FastAPI 覆盖：项目创建、项目列表、项目详情、大纲与人物卡生成、指定章节生成、流式预览、generation status、章节阅读、TXT 导出、Narrative Graph、Context Pack、Story Delta、Knowledge Draft Review & Merge、项目 Generation Settings、项目删除。
...
项目默认模型为 `deepseek-v4-flash`，Base URL 默认为 `https://api.deepseek.com`。当前正式前端在“设置 → 项目设置”页支持编辑当前项目的 `model`、`max_tokens` 和 `temperature`。
...
> **重要**：`deepseek-v4-flash` 是推理模型，流式生成正文时先输出推理过程再输出正文。`max_tokens` 低于约 8192 时推理会耗尽预算导致只有推理、无正文（报错 "Model stream ended without final content"）。后端默认 `DEFAULT_MAX_TOKENS` 已设为 **16384**（章节生成与续写一致），建议前端创建 / 编辑项目时保持 ≥ 8192。
...
- 叙事图谱已支持 G6 画布可视化：拖拽布局、节点 / 关系编辑、从大纲一键导入。

- 章节生成与对话式续写支持 AI 推理过程展示（reasoning 事件），推理内容不写入任何文件。

- 对话式续写支持一键保存回章节文件（append / replace）。

- 侧边栏导航：仪表盘、创作驾驶舱、阅读器、叙事图谱、章节审查、资料库、设置。

- 开屏动画可通过 `frontend/src/appConfig.ts` 的 `APP_CONFIG.showIntro` 或 `VITE_SHOW_INTRO` 环境变量控制，默认关闭。
...
- 图谱布局坐标持久化（当前拖拽布局刷新后回到自动布局）。

- 系统级 API Key / 模型连接测试 UI。
