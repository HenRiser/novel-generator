...
- Story Delta prompt now also applies a conservative fact-compression guard: candidate changes should distinguish explicit facts from hints, speculation, partial records, and possible links; they should not rewrite disease names, dates, years, death timing, voting timing, investigation targets, organization actions, or causality. This is prompt guidance only, not a full fact-checking system, and it does not automatically fix or reject candidates.
