...
@router.post("/import-assets", response_model=NarrativeGraphResponse)

def import_outline_assets(project_ref: str) -> NarrativeGraphResponse:

    """从大纲自动导入图谱节点（主要人物 → character，世界观设定 → world_fact）。



    跳过已存在的同名节点，不覆盖手动编辑数据。

    """

    result = import_assets_to_graph(project_ref)

    if not result.ok:

        _error(_status_for_message(result.message), "narrative_graph_import_failed", result.message)
