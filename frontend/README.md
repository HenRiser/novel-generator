...
Context Pack preview in React calls `POST /api/projects/{project_ref}/context-pack/preview`. The preview reads the current Narrative Graph, returns a structured pack plus prompt text, and does not call the model or write graph/chapter files. The UI now shows a creator-focused preview that groups selected context into characters, events/scenes, foreshadowing, world rules, plot directions, narrative relationships, and high-priority reminders for the next chapter. Raw Prompt / Debug remains available. The selection algorithm is unchanged, while generated prompt text now separates selected `status=confirmed` records with `importance >= 8` into Hard Continuity Constraints before lower-priority confirmed facts and background context. When the injected Context Pack `Chapter goal` explicitly asks for low-intensity pacing, emotional digestion, or no new settings / archives / file numbers / paper notes, chapter generation appends a Low-Intensity Chapter Constraints block. This is prompt guidance only and does not change Context Pack selection, prompt rendering, Story Delta, Review & Merge, or Graph writes. The generation-panel usage state is UI-only: it does not change Context Pack selection, prompt rendering, Story Delta, Review & Merge, or Graph writes. The post-generation consistency warning pass only checks explicit date, life/death, identity, and organization-affiliation conflicts; it is not a full Consistency Policy / Conflict Detection system.
...
Knowledge Draft review in React uses `GET /api/projects/{project_ref}/knowledge-drafts`, `GET /api/projects/{project_ref}/knowledge-drafts/{draft_id}`, `POST /api/projects/{project_ref}/knowledge-drafts/{draft_id}/changes/{change_id}/accept`, and `POST /api/projects/{project_ref}/knowledge-drafts/{draft_id}/changes/{change_id}/reject`. The Review panel defaults to the latest pending Knowledge Draft, falls back to the latest draft when none are pending, and preserves user selection during refresh. Accepting is intentionally limited to `create_node` and `create_edge`; rejecting is available for unsupported operations too. This is a review foundation, not AI auto-review, batch merge, character-card merge, duplicate resolution, or conflict detection.

Story Delta candidate changes now prefer directly reviewable operations. New `create_node` payloads use `type`, not `node_type`; legacy `node_type` payloads remain acceptable for old drafts. First-order world fact, foreshadowing, plot direction, and character-card candidates are normalized into `create_node` when possible. `create_edge` candidates can point to existing graph nodes or same-draft node candidates through `source_change_id` and `target_change_id`. Future Story Delta candidates use a conservative importance rubric: most changes should be 4-7, 8-10 are rare high-priority continuity constraints, and confirmed does not automatically mean high importance. They also use conservative fact-compression and uncertainty-status guards: candidates should distinguish explicit facts from hints, speculation, partial records, and possible links; summaries with markers such as possible, suspected, hinted, unclear, or possible link should usually avoid `confirmed` and prefer `unresolved`, `partially_revealed`, `introduced`, `active`, or `planned` when appropriate. Candidates should not rewrite disease names, dates, years, death timing, voting timing, investigation targets, organization actions, or causality. This does not migrate historical Graph data, automatically fix candidates, or change Context Pack selection / Hard Constraints extraction. Timeline Review, Health Dashboard, Future Outline Revision, Consistency Policy, and Advanced Review & Merge remain unimplemented.
