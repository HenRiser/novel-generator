...
The creation page now includes Context Pack Builder foundation support. It previews a structured Narrative Context Pack selected from the Narrative Graph by local rules, lets the user control `min_importance`, node and edge limits, unresolved foreshadowing, and neighbor inclusion, and keeps context-assisted generation disabled by default. The chapter generation panel shows whether the current run will inject the previewed Context Pack, plus selected node, relationship, and hard-constraint counts. After generation is saved, React can display lightweight consistency warnings for explicit date, life/death, identity, and organization-affiliation conflicts against the injected Hard Continuity Constraints. These warnings are non-blocking and do not rewrite prose.
